...
#         mu ~ dlnorm(1, 0.2)
        ...
        Beta.pH[1] ~ dnorm(0, 0.01)
        Beta.pH[2] ~ dnorm(0, 0.01)
        Beta.DOC ~ dnorm(0, 0.01)
...
CaAl.constants <- list(pH = chem.spec[,pH],
                       DOC = chem.spec[,DOC],
                       Ca.Al = chem.spec[,Ca.Al],                       
...
save(caal.mcmc, file="Ca_Al.Rdata")
...
for(i in 1:n.chains){
        ...
        ca.mcmc[[i]] <- mcmc(as.matrix(Al.proj$Al.MCMC$mvSamples)[bt,])
}
al.mcmc <- mcmc.list(al.mcmc)
save(al.mcmc, file = "Al.Rdata")
